...
export function Admin() {
  ...
  async function fetchAdminData() {
    try {
      ...
      if (profiles) setUsers(profiles as AdminProfile[]);

      const { data: loans } = await supabase
        .from('loans')
        .select('principal_amount, status');

      const safeLoans = (loans || []) as LoanSummary[];
      const activeLoans = safeLoans.filter((loan) => loan.status === 'active');
      const totalVolume = safeLoans.reduce((acc, curr) => acc + Number(curr.principal_amount), 0);
  ...
  async function toggleUserPlan(userId: string, currentPlan: string) {
    ...
    const currentIndex = plans.indexOf((currentPlan as 'free' | 'pro' | 'enterprise') || 'free');
    const nextPlan = plans[((currentIndex >= 0 ? currentIndex : 0) + 1) % plans.length];
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-center">
        <div className="space-y-6 max-w-md">
          ...
          <p className="text-slate-500 font-medium">{t.accessDenied}</p>
          <button
            onClick={() => navigate('/')}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input type="number" defaultValue={97.0} className="w-20 bg-white border border-emerald-200 rounded-lg px-2 py-1 text-emerald-900 font-bold" />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-4 bg-violet-50 rounded-2xl border border-violet-100">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input type="number" defaultValue={497.0} className="w-20 bg-white border border-violet-200 rounded-lg px-2 py-1 text-violet-900 font-bold" />
                        </div>
                      </div>
                    </div>
