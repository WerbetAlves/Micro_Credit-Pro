...
type AdminProfile = {
  ...
  created_at: string;
  plan_type: 'free' | 'start' | 'pro' | 'business' | 'enterprise';
  is_admin?: boolean;
};
...
export function Admin() {
  ...
  async function toggleUserPlan(userId: string, currentPlan: string) {
    const plans: ('free' | 'start' | 'pro' | 'business')[] = ['free', 'start', 'pro', 'business'];
    const currentIndex = plans.indexOf((currentPlan as 'free' | 'start' | 'pro' | 'business') || 'free');
    ...
    try {
      ...
      const response = await fetch('/api/admin/update-user-plan', {
        ...
        body: JSON.stringify({ userId, planType: nextPlan }),
      });
      ...
      setAdminError(err instanceof Error ? err.message : 'Falha ao atualizar plano.');
    }
  }
                          ...
                          <tr key={u.id} className="hover:bg-slate-50/30 transition-colors">
                            ...
                            <td className="px-4 lg:px-8 py-5">
                              <div className={cn(
                                ...
                                u.plan_type === 'start' ? "bg-amber-100 text-amber-700" :
                                "bg-slate-100 text-slate-600"
                              )}>
                                {(u.plan_type === 'business' || u.plan_type === 'enterprise') && <Shield className="size-3" />}
