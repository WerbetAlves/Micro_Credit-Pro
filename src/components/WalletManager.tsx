...
import {
  Wallet, Plus, X, Landmark, Banknote, Smartphone, ChevronRight, Trash2, Edit2, ShieldCheck, RefreshCw, Search, Zap
} from 'lucide-react';
...
export function WalletManager() {
  ...
  const handleConnectBank = async (bank: typeof POPULAR_BANKS[number]) => {
    if (!user) return;
  ...
  const handleOpenFinanceClick = () => {
    alert('Open Finance ainda está em desenvolvimento. Em breve você poderá conectar seu banco por aqui.');
  };
  ...
  const getIcon = (walletType: WalletData['type']) => {
    switch (walletType) {
      case 'physical':
  ...
  const getWalletTypeLabel = (walletType: WalletData['type']) => {
    if (walletType === 'physical') return t.physicalWallet;
    if (walletType === 'bank') return t.bankAccount;
        ...
        {wallets.map((wallet) => (
          ...
          >
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{getWalletTypeLabel(wallet.type)}</p>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {[
                          ...
                          <button
                            key={opt.id}
                            type="button"
                            onClick={() => setType(opt.id as WalletData['type'])}
                            className={cn(
