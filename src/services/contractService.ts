...
export async function generateLoanContract(loanData: any, clientData: any, userData: any) {
  const prompt = `
    ...
    - Frequência de Pagamento: ${
      loanData.payment_frequency === 'daily'
        ? 'Diário'
        : loanData.payment_frequency === 'weekly'
          ? 'Semanal'
          : loanData.payment_frequency === 'biweekly'
            ? 'Quinzenal'
            : 'Mensal'
    }
    ...
    - Garantia: ${
      loanData.guarantee_info
        ? `${loanData.guarantee_info.type} - ${loanData.guarantee_info.description}`
        : 'Nenhuma'
    }
  ...
  try {
    ...
    if (response.type === 'text' || response.type === 'message') {
      return response.text;
    }

    return 'Não foi possível gerar o contrato automaticamente.';
  } catch (error) {
    console.error('Error generating contract with OpenAI backend:', error);
